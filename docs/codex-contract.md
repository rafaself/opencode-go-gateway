...
The decoder returns the explicit unions in `internal/bridge`: ordered messages with `input_text` content, function/custom calls and outputs, function tools with immutable raw JSON Schema, explicit tool choice, generation options, and known deferred tool markers. Tool outputs preserve exact text and reduce optional status/error markers to semantic metadata. The bridge package has no dependency on Responses or OpenAI-specific wire structs; `internal/opencodego` owns provider translation, SSE handling, and the bounded M8 continuation state.
